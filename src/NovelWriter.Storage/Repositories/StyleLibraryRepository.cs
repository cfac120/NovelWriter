...
public class StyleLibraryRepository : IStyleLibraryRepository
{
    ...
    public async Task<StyleProfile?> GetRandomUnUsedInVolumeAsync(int volumeNumber)
    {
        var usedIds = await _db.StyleUsageLogs
            .Where(l => l.VolumeNumber == volumeNumber)
            .Select(l => l.StyleId)
            .ToListAsync();
    ...
    public async Task LogUsageAsync(string styleId, int volumeNumber, int chapterNumber)
    {
